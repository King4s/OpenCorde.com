<script lang="ts">
	...
	if (browser) {
		...
		spaceId = sid;
		if (cid) {
			forumStore.fetchPosts(cid).catch(() => {});
		}
	}
	...
	function goToPost(postId: string) {
		window.location.href = `/servers/${spaceId}/forum/${channelId}/${postId}`;
	}
...
<div class="flex-1 flex flex-col bg-gray-900">
	...
	<div class="h-12 px-6 flex items-center justify-between border-b border-gray-800">
		...
		<button
			...
			class="px-4 py-2 bg-gray-600 hover:bg-gray-700 text-white text-sm rounded font-medium transition-colors"
		>
			+ New Post
		</button>
	</div>
	...
	{#if showNewPostForm}
		<div class="px-6 py-4 bg-gray-850 border-b border-gray-800">
			{#if createError}
				<p class="text-gray-400 text-sm mb-3">{createError}</p>
			{/if}
			<input
				type="text"
				bind:value={newPostTitle}
				placeholder="Post title"
				class="w-full px-3 py-2 bg-gray-900 border border-gray-700 rounded text-white text-sm placeholder-gray-500 focus:outline-none focus:border-gray-500 mb-2"
			/>
			<textarea
				bind:value={newPostContent}
				placeholder="What's on your mind?"
				rows={4}
				class="w-full px-3 py-2 bg-gray-900 border border-gray-700 rounded text-white text-sm placeholder-gray-500 focus:outline-none focus:border-gray-500 resize-none mb-3"
			></textarea>
			...
			<div class="space-y-2 p-4">
				{#each forumStore.posts as post (post.id)}
					<button
						...
						class="w-full text-left p-4 bg-gray-800 hover:bg-gray-750 rounded border border-gray-700 hover:border-gray-500 transition-all group"
					>
						<div class="flex items-start justify-between mb-1">
							<h3 class="font-semibold text-white group-hover:text-gray-400 transition-colors">{post.title}</h3>
							{#if post.pinned}
								<span class="text-xs bg-gray-600 text-white px-2 py-1 rounded">Pinned</span>
							{/if}
						</div>
						...
						<div class="flex items-center justify-between text-xs text-gray-400">
							...
							<span class="text-gray-400">{post.reply_count} {post.reply_count === 1 ? 'reply' : 'replies'}</span>
						</div>
					</button>
				{/each}
			</div>

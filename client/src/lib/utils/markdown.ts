...
export function renderMarkdown(text: string): string {
	...
	processed = processed.replace(/(?<!_)_(?!_)(.+?)(?<!_)_(?!_)/gs, '<em>$1</em>');
	...
	processed = processed.replace(/^&gt; (.+)$/gm, '<div class="border-l-4 border-gray-500 pl-2 my-1 text-gray-400">$1</div>');
	...
	processed = processed.replace(/\x00INLINE(\d+)\x00/g, (_, i) => inlineCodes[parseInt(i)]);

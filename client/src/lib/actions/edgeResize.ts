...
const DEFAULT_OPTIONS: Required<Pick<Options, 'handles' | 'minWidth' | 'maxWidth' | 'handleSize' | 'visible'>> = {
	handles: ['right'],
	minWidth: 160,
...
export const edgeResize: Action<HTMLElement, Options> = (node, opts = {}) => {
	...
	handle.style.background = options.visible ? 'linear-gradient(to right, rgba(0,0,0,0.16), rgba(255,255,255,0.10), rgba(0,0,0,0.16))' : 'transparent';
	handle.style.opacity = '0.9';
	handle.style.transition = 'background-color 120ms ease, box-shadow 120ms ease, transform 120ms ease';
	handle.style.boxShadow = 'inset 0 0 0 1px rgba(255,255,255,0.12), inset 0 0 0 2px rgba(0,0,0,0.22)';
	handle.style[edge] = `-${Math.floor(options.handleSize / 2)}px`;
		...
		handle.addEventListener('pointerdown', (event) => {
			...
			const onMove = (moveEvent: PointerEvent) => {
				...
				const nextWidth = edge === 'right' ? initialWidth + delta : initialWidth - delta;
		...
		indicator.style.boxShadow = '0 0 0 1px rgba(0, 0, 0, 0.55), 0 1px 2px rgba(0, 0, 0, 0.45)';
	...
	return {
		update(nextOpts = {}) {
			...
			if (next.minWidth !== options.minWidth || next.maxWidth !== options.maxWidth) {

...
export const friendStore = {
	...
	async fetchPending() {
		const data = await api.get<{ incoming: Relationship[]; outgoing: Relationship[] }>(
			'/friends/pending'
		);
	...
	async sendRequest(userId: string) {
		const rel = await api.post<Relationship>('/friends/request', { user_id: userId });
	...
	async search(query: string) {
		...
		searchResults = await api.get<UserSearchResult[]>(`/users/search?q=${encodeURIComponent(query)}`);
	},

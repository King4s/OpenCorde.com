...
export const eventStore = {
  ...
  async fetchForServer(spaceId: string, past = false) {
    ...
    try {
      ...
      events = await api.get<ServerEvent[]>(
        `/servers/${spaceId}/events${query}`,
      );
    } catch (e: any) {
  ...
  async create(
    ...
    const event = await api.post<ServerEvent>(
      `/servers/${spaceId}/events`,
      data,
    );
    events = [event, ...events];
    return event;
  ...
  async unRsvp(eventId: string) {
    ...
    events = events.map((e) =>
      e.id === eventId
        ? { ...e, rsvp_count: Math.max(0, e.rsvp_count - 1) }
        : e,
    );
  },
  ...
  async updateStatus(
    ...
    events = events.map((e) => (e.id === eventId ? { ...e, status } : e));
  },

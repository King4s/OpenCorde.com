...
export const textChannels = derived(channels, ($ch) =>
  $ch.filter((c) => c.channel_type === 0),
);
export const voiceChannels = derived(channels, ($ch) =>
  $ch.filter((c) => c.channel_type === 1),
);
export const stageChannels = derived(channels, ($ch) =>
  $ch.filter((c) => c.channel_type === 3),
);
export const forumChannels = derived(channels, ($ch) =>
  $ch.filter((c) => c.channel_type === 5),
);
...
export function initChannelListeners(): void {
  ...
  gateway.on("ChannelUpdate", (data: unknown) => {
    ...
    channels.update((list) =>
      list.map((c) => (c.id === event.channel.id ? event.channel : c)),
    );
  });
...
export async function createChannel(
  ...
  const channel = await api.post<Channel>(`/servers/${spaceId}/channels`, {
    name,
